		...
		<div>
			...
			<div class="flex bordera text-center items-center justify-around relative bottom-[145px]">
				...
				<div class="h-[56px] w-[42%] bordera text-center bg-white flex rounded-md mr-3" @click="add(1)">
					<img class="w-[50px] h-[50px] mt-[1px]" src="https://my-xl.oss-cn-beijing.aliyuncs.com/images/settings.png" />
					<div class="flex-1 bordera flex items-center font-semibold font-[20px] ml-2" style="color: #409eff">+添加保修</div>
				</div>
			</div>
		</div>
					...
					<div v-for="item in list" :key="item.id" class="bg-white borderb mt-4 rounded-[10px] h-[275px]">
						<div class="flex justify-between">
							...
							<div class="text-center mt-[6px] mr-4">
								...
								<div v-if="item.state == 1" class="bg-yellow-500 radiusButton text-[10px] text-white">处理中</div>
								<div v-if="item.state == 2" class="radiusButton text-white text-[10px]" style="background-color: #409eff">已理中</div>
								<div v-if="item.state == 3" class="bg-gray-300 radiusButton text-[10px] text-white">已评价</div>
							</div>
						</div>
